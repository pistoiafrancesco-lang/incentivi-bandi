...
-------------------------------------------------
Richiede POST multipart/form-data.
...
def fetch_page(page: int) -> dict:
    """Scarica una pagina di risultati via POST multipart/form-data."""
    params = {
        "apiKey":     "SEDIA",
        "text":       "*",
        "pageSize":   PAGE_SIZE,
    ...
    # Filtro ElasticSearch: solo bandi aperti o in apertura
    es_query = json.dumps({
        "bool": {
            "filter": [
                {"terms": {"status": ["open", "forthcoming"]}}
            ]
        }
    })
    # Il campo "query" va inviato come file con content-type application/json
    files = {
        "query": ("query.json", es_query, "application/json")
    }
    headers = {
        "User-Agent": "Mozilla/5.0 (compatible; EUScraper/1.0)"
    }
...
def fetch_all() -> list[dict]:
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        print(f"    → {len(hits)} bandi, totale API: {total}")
