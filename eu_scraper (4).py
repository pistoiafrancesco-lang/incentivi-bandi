...
Filtro ES con wrapper {"query": {"bool": ...}} corretto.
...
def build_es_query() -> str:
    """Struttura ES corretta con wrapper 'query'."""
...
def is_active(chiusura: str) -> bool:
    if not chiusura:
        return True
    try:
...
def fetch_all() -> list[dict]:
    ...
    print(f"  Query ES: deadlineDate >= {TODAY.date()}")
    all_records = []
    total_pages = 1
    ...
    for page in range(1, MAX_PAGES + 1):
        ...
        for item in hits:
            r = map_record(item)
            if is_active(r["data_chiusura"]):
                all_records.append(r)
                page_active += 1

        total = data.get("totalResults", 0)
        total_pages = min(MAX_PAGES, -(-total // PAGE_SIZE))
        print(f"    → {len(hits)} ricevuti, attivi: {page_active}, totale API: {total}")
...
def save_outputs(rows: list[dict]):
    ...
    fname_csv = f"eu_bandi_{ds}.csv"
    if OUTPUT_CSV:
        if rows:
            with open(fname_csv, "w", newline="", encoding="utf-8-sig") as f:
                w = csv.DictWriter(f, fieldnames=list(rows[0].keys()))
                w.writeheader()
                w.writerows(rows)
        else:
            with open(fname_csv, "w", encoding="utf-8-sig") as f:
                f.write("id,titolo,data_chiusura,programma\n")
        print(f"✓ CSV:  {fname_csv} ({len(rows)} bandi)")
...
def print_summary(rows: list[dict]):
    ...
    if rows:
        ...
        for p, c in sorted(programmi.items(), key=lambda x: -x[1])[:8]:
            print(f"    {c:4d}  {p}")
        # Prima e ultima scadenza
        date_vals = [r["data_chiusura"] for r in rows if r.get("data_chiusura")]
        if date_vals:
            print(f"\n  Prima scadenza: {min(date_vals)[:10]}")
            print(f"  Ultima scadenza: {max(date_vals)[:10]}")
    print(f"{'─'*50}\n")
